...
goal_point_x = 4.0
goal_point_y = 0.0
goal = np.array([goal_point_x , goal_point_y ])
...
x1 = np.linspace(0, 10 , 10) + x_postition
y1 = np.linspace(0, 10, 10) + y_postition
...
goal_marker = ax.plot(goal[0], goal[1], 'gx')[0]
...
def timer_func( node, st_pub , thr_pub ) : 
    global  postition , car_yaw 
    st = Float32()
    thr = Float32()

    node.get_logger().info("Publishing : >_<" )
    if np.isclose(goal_point_x, postition[0], atol=0.1) and np.isclose(goal_point_y, postition[1], atol=0.1):
    ...
    xy_cf = transformation( postition ,goal ,car_yaw )
